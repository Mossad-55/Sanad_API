...
- **Admin elderly medication operational reads**: Normal SuperAdmin/SupportAdmin prescription, persisted dose-log, and adherence reads; every successful sensitive GET writes an immutable payload-free audit row (`docs/admin/elderly-medications.md`)
- **Elderly wellness tips**: Normal Elderly published-only paged list/detail plus SuperAdmin/ContentAdmin Normal CMS list/search/detail/preview, multipart draft authoring with generated image storage keys, publish, and archive (`docs/app/elderly/wellness-tips.md`, `docs/admin/wellness-tips.md`). CMS content is currently global/shared; tenant scope and clinical review remain Needs Owner Verification.
- **Elderly profile**: Normal Elderly-only `GET /api/v1/elderly/profile` and `/photo` resolve the linked dependent from JWT identity, return bilingual names, profile-local computed age, photo state/path, latest linked assessment, and stored IANA timezone (`docs/app/elderly/profile.md`). Family Owners may set a dependent timezone; dependent responses include `timeZoneId`.
- **Next subscription slices**: booking-completion allowance consumption and notifications/email. Family-owner immediate prorated upgrades and next-renewal downgrades are implemented; successful initial and renewal settlement now creates immutable branded invoice PDFs with owner-only list/detail/download routes; provider subscription callback settlement, shared identity persistence, and seven-day grace handling are implemented; Wallet remains one-time/manual.
...
| POST | `/family/subscriptions/quote` | Server-calculated tax-inclusive purchase quote (Owner only; read-only) |
| POST | `/family/subscriptions/payment-intent` | Start initial Card/Wallet checkout from a server quote (Owner only) |
...
- Subscriptions: `docs/admin/subscriptions.md` — Super Admin plan list/detail, family subscription list/detail, plan authoring including optional Paymob mapping, one-time publication, retirement, coupon create/list/detail/hard-delete configuration, and tax-rule create/current/history; family owners can request server-calculated purchase and plan-change quotes, initial Card enrollment when configured, immediate upgrades, pending next-renewal downgrades, manual renewal payment intents, Paymob callback settlement with durable identity/idempotency, renewal-grace recovery, anchor-preserving payment retry, and immutable invoice list/detail/private PDF downloads. Allowance consumption follows it.
- Elderly medication operations: `docs/admin/elderly-medications.md` — SuperAdmin and SupportAdmin with Normal access can list/detail prescriptions, inspect persisted dose timelines (inclusive ranges up to 31 days), and read persisted-dose adherence aggregates. No write APIs; successful sensitive reads audit before returning data. Postman/Bruno examples are manual/stateful and must not run against shared/live environments.
