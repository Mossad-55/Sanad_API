...
Routes for creating a care booking (checkout), browsing requests by tab, reading booking details, and cancelling. The family pays the caregiver's price plus a platform fee; the caregiver then accepts or declines.
...
- **List / detail / cancel** are available to every family member — but only for bookings of their **own** family (`404 Bookings.BookingNotInFamily` for a foreign booking, `404 Bookings.FamilyNotFound` when the caller belongs to no family).
...
- **Acceptance window:** the caregiver must respond within `min(now + 24h, booking start)`. Payment and acceptance past the deadline are rejected; a pending booking that passes the deadline ends as `Expired` (status `10`).
...
- `paymentClientSecret` is currently a **development stub** (`paymob_sim_secret_{bookingId}`). Real Paymob verification (HMAC-signed, idempotent webhook) ships in the payments slice; until then the payment-confirmation command is not exposed over HTTP.
...
```json
{
  ...
  "currency": "EGP",
  "paymentClientSecret": "paymob_sim_secret_0198e3f0-3333-7777-8888-000000000003"
}
```

`totalPayableAmount = caregiver base fee + 15% platform fee` (e.g. base `2,500.00` → total `2,875.00`).
...
| `Bookings.BookingNotInFamily` | 404 | Booking belongs to another family. |
| `Bookings.Domain.InvalidOperation` | 409 | Lifecycle rule violated (wrong-status transition, cancel after completion, past-deadline pay/accept). |
